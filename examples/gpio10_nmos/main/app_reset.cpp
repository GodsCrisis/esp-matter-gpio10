...
esp_err_t app_reset_button_register()
{
    ...
    gpio_button_config_t gpio_cfg = {
        .gpio_num = RESET_BUTTON_GPIO,
        .active_level = 0,
    };

    extern const button_driver_t button_gpio_driver;
    esp_err_t err = iot_button_create(&button_cfg, &button_gpio_driver, &gpio_cfg, &button_handle);
